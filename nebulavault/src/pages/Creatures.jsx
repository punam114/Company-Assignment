...
import { Plus, Search, ChevronLeft, ChevronRight, Loader2, User, Info } from 'lucide-react';
...
export default function Creatures({ isFrozen }) {
    ...
    const { currentPage, filter, setCurrentPage, setFilter } = useTabState('creatures');
    ...
    async function fetchData() {
        try {
            const res = await axios.get(`${BASE_URL}/creatures.json`);
            const val = res.data;
    ...
    const handleAdd = async (e) => {
        ...
        setIsAdding(true);
        try {
            await axios.post(`${BASE_URL}/creatures.json`, {
                name: newItemName,
                timestamp: Date.now(),
                creator: 'Observer-01',
                type: 'Entity'
    ...
    if (error) return <div style={{ color: '#ef4444', padding: '20px' }}>Error loading Creatures: {error.message}</div>;

    return (
        <div>
            <div style={{ paddingBottom: '16px', borderBottom: '1px solid #d1d5db', marginBottom: '20px' }}>
                <h3 style={{ marginBottom: '12px' }}>Creatures Explorer</h3>
                <div style={{ position: 'relative', marginBottom: '16px' }}>
                    <Search size={16} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#6b7280', pointerEvents: 'none' }} />
                    <input
                        type="text"
                        placeholder="Search creatures by name, ID, creator, or type..."
                        value={filter}
                ...
                <form onSubmit={handleAdd} style={{ display: 'flex', gap: '8px' }}>
                    <input
                        type="text"
                        placeholder="Enter new creature name..."
                        value={newItemName}
            ...
            <div>
                {filter && (
                    <div style={{ 
                        ...
                        Found {filteredData.length} creature{filteredData.length !== 1 ? 's' : ''} matching "{filter}"
